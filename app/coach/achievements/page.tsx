...
export default function AchievementsTab() {
  ...
  useEffect(() => {
    ...
    return () => clearTimeout(delay);
  }, [studentSearch, branchId]);

  useEffect(() => {
    if (branchId) {
      setFilteredSports(sports.filter((s) => s.branchId === branchId));
  ...
  const handleCreate = async () => {
    ...
    await axiosInstance.post(`${API}/sports/achievements`, {
      ...form,
      branchId,
    });
    resetForm();
  ...
  const handleUpdate = async () => {
    ...
    await axiosInstance.put(`${API}/sports/achievements/${editingId}`, {
      ...form,
      branchId,
    });
    setEditOpen(false);
  ...
  const renderForm = (isEdit = false) => {
    ...
    return (
      <div className="space-y-3 mt-4">
        {/* Branch is auto-selected */}
        <div className="p-2 bg-blue-50 rounded-lg border border-blue-100 text-blue-700">
          Branch configured in localStorage: {branchId || "Not selected"}
        </div>
        ...
        <div className="border rounded-md max-h-40 overflow-y-auto">
          {!branchId ? (
            <p className="p-2 text-sm text-muted-foreground">
              Branch not selected. set branchId in localStorage.
            </p>
          ) : selectedStudent ? (
