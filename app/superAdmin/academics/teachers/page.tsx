...
function TeachersComponent({ highlightId }: { highlightId: string | null }) {
  ...
  const effectiveBranchId = isGlobalBranchSelected
    ? selectedBranchId
    : branchFilter !== "all"
      ? branchFilter
      : undefined;
  ...
  /* ---------------- FETCH DATA ---------------- */
  useEffect(() => {
    fetchTeachers(effectiveBranchId ?? undefined);
    fetchBranches();
    fetchSubjects();
  }, [effectiveBranchId]);
  ...
  useEffect(() => {
    ...
    } else if (branchFilter === selectedBranchId) {
      setBranchFilter("all");
    }
  ...
  useEffect(() => {
    if (isAddModalOpen) {
      ...
      fetchClasses(branchIdToUse);
      fetchSections(branchIdToUse);
    }
  }, [isAddModalOpen, selectedBranch, selectedBranchId]);
  ...
  const fetchTeachers = async (branchId?: string) => {
    try {
      ...
      const res = await axiosInstance.get("/api/v1/teachers", { params });
  ...
  const fetchClasses = async (branchId?: string) => {
    try {
      ...
      const res = await axiosInstance.get("/api/v1/classes", { params });
      const classesData = res?.data?.data?.classes || res?.data?.data || [];
      setClasses(
        classesData.map((cls: any) => ({
          id: String(cls.id),
          name: cls.name,
          branchId: String(cls.branchId),
        })),
      );
    } catch {
  ...
  const fetchSections = async (branchId?: string) => {
    try {
      ...
      const res = await axiosInstance.get("/api/v1/sections", { params });
      const sectionsData = res?.data?.data?.sections || res?.data?.data || [];
      setSections(
  ...
  const filteredClasses = classes.filter(
    (c) => c.branchId && c.branchId === formData.branchId,
  );
  const filteredSections = sections.filter(
    (s) => s.classId && s.classId === formData.classId,
  );
      ...
      <AdminLayout>
        ...
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between mb-5">
          ...
          <div className="w-full sm:w-[240px]">
            ...
            >
              <SelectTrigger
                className={isGlobalBranchSelected ? "bg-muted/10" : ""}
              >
                ...
                >
                  ...
                  <TableCell>
                    <div className="flex items-center gap-2">
                      ...
                      {branches.find((b) => String(b.id) === String(t.branchId))
                        ?.name || "-"}
                    </div>
                  </TableCell>
                  ...
                  <TableCell>
                    <div className="flex items-center gap-2">
                      ...
                      {subjects.find(
                        (s) => String(s.id) === String(t.subjectId),
                      )?.name || "-"}
                    </div>
                  </TableCell>
            ...
            <div className="space-y-4 px-6 py-5">
              ...
              <div className="relative">
                ...
                <Select
                  ...
                  onValueChange={(v) =>
                    setFormData({ ...formData, classId: v, sectionId: "" })
                  }
