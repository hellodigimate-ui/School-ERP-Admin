...
function HRPage({ highlightId }: { highlightId: string | null }) {
  ...
  const downloadAttendancePDF = () => {
    ...
    const empRecords = attendanceData.filter((a) => a.user?.email === empEmail);
  ...
  const fetchStaff = async () => {
    try {
      ...
      const branchIdToUse = isGlobalBranchSelected
        ? selectedBranchId
        : localBranch !== "all"
          ? localBranch
          : undefined;
      ...
      setTotalStaff(res.data?.pagination?.totalStaff || 0);
  ...
  return (
    <AdminLayout>
        ...
        <div className="relative flex flex-wrap items-center justify-between gap-6">
          ...
          <div>
            ...
            <p className="text-white/80 mt-2 max-w-xl">
              Manage staff directory, attendance, leave requests, departments
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab}>
          ...
          <div className="bg-card rounded-xl border border-border p-6">
            ...
            <div className="mt-4 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
              <p className="text-sm text-muted-foreground">
                ...
                {Math.min(currentPage * perPage, totalStaff)} of {totalStaff}{" "}
                staff
              </p>
            ...
            <div className="flex items-center justify-between mb-4 flex-wrap gap-3">
              ...
              <div className="flex items-center gap-2 flex-wrap">
                ...
                <Select
                  ...
                  onValueChange={setAttendanceRoleFilter}
                >
              ...
              const filtered = attendanceData.filter(
                (a) =>
                  ...
                  (attendanceRoleFilter === "all"
                    ? true
                    : a.user?.role === attendanceRoleFilter),
              );
                ...
                <>
                  ...
                  <div className="grid grid-cols-4 gap-4 mb-6">
                    {[
                      {
                        label: "Present",
                        count: filtered.filter((a) => a.status === "P").length,
                        icon: CheckCircle,
                        color: "text-emerald-600 bg-emerald-50",
                      },
                      {
                        label: "Absent",
                        count: filtered.filter((a) => a.status === "A").length,
                        icon: XCircle,
                        color: "text-red-600 bg-red-50",
                      },
                      {
                        label: "On Leave",
                        count: filtered.filter((a) => a.status === "L").length,
                        icon: Calendar,
                        color: "text-amber-600 bg-amber-50",
                      },
                      {
                        label: "Late",
                        count: filtered.filter((a) => a.status === "LT").length,
                        icon: Clock,
                      ...
                      <div
                        key={s.label}
                        className={`rounded-xl p-4 flex items-center gap-3 ${s.color}`}
                      >
                        <s.icon size={24} />
                        <div>
        ...
        <Dialog
          open={attendanceDetailOpen}
          onOpenChange={setAttendanceDetailOpen}
        >
          <DialogContent className="max-w-2xl max-h-96 overflow-y-auto">
            ...
            {selectedAttendanceRecord &&
              (() => {
                const empName =
                  selectedAttendanceRecord.user?.name || "Unknown";
                const empRole =
                  selectedAttendanceRecord.user?.role || "Unknown";
                const empEmail = selectedAttendanceRecord.user?.email || "N/A";
                const empCode = selectedAttendanceRecord.empCode || "N/A";
                ...
                const leaveCount = employeeRecords.filter(
                  (a) => a.status === "L",
                ).length;
