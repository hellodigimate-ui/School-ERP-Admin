...
const getStats = (data: any[]) => {
  ...
  const absentToday = studentData.filter(a => a.status === "A").length;
  const accuracy = uniqueStudents > 0 ? ((presentToday / uniqueStudents) * 100).toFixed(1) : 0;
  ...
  return [
    ...
    {
      label: "Facial Data",
      value: (uniqueStudents > 0 ? (presentToday + absentToday).toString() : "0"),
      icon: Camera,
      gradient: "from-blue-500 to-cyan-500",
      bg: "bg-secondary/50",
      change: "Records today",
    },
  ];
};
...
const Page = () => {
  ...
  const downloadPDF = () => {
    ...
    // Header
    doc.setFontSize(16);
    doc.text("Student Attendance Report", 10, yPos);
    yPos += 10;
