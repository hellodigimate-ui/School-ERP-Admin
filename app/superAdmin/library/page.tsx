...
const Page = () => {
  ...
  useEffect(() => {
    fetchIssues();
  }, [issuePage, branchFilter]);

  const fetchIssues = async () => {
    try {
      setIssueLoading(true);

      const res = await axiosInstance.get("/api/v1/books/issue", {
        params: {
          page: issuePage,
  ...
  }, [page, perPage, searchTerm, branchFilter]);
  ...
  const handleUpdateBook = async (bookId, updatedBook) => {
    try {
      const payload = { ...updatedBook };
      if (payload.branchId === "all" || !payload.branchId) {
        delete payload.branchId;
      }

      const res = await axiosInstance.put(`/api/v1/books/${bookId}`, payload);
    ...
    axiosInstance
      ...
      .then((res) => {
        ...
        setStats([
          ...
          {
            label: "Available Books",
            value: data.totalAvailableBooks,
            icon: BookIcon,
            gradient: "from-amber-500 to-orange-500",
      ...
      <div className="space-y-6">
        <div>
          ...
          <p className="text-muted-foreground mt-1">
            Manage books, issue/return, members & fines
          </p>
        </div>
                  ...
                  <div className="overflow-y-auto px-6 py-5 space-y-5 bg-card dark:bg-card">
                    <div className="bg-secondary/50 dark:bg-secondary/50 border border-border dark:border-border rounded-xl p-4 space-y-4">
                      ...
                      <div className="space-y-1">
                        ...
                        <Select
                          value={newBook.branchId || branchFilter || "all"}
                          onValueChange={(value) =>
                            setNewBook({ ...newBook, branchId: value })
                          }
                          disabled={isGlobalBranchSelected}
                        >
                          <SelectTrigger className="w-full">
                    ...
                    <Button
                      ...
                      onClick={async () => {
                        try {
                          ...
                          const selectedBranchIdForBook =
                            newBook.branchId && newBook.branchId !== "all"
                              ? newBook.branchId
                              : branchFilter;
                          if (selectedBranchIdForBook) {
                            payload.branchId = selectedBranchIdForBook;
                          }
                                  ...
                                  <div className="p-6 space-y-5 bg-card dark:bg-card">
                                    ...
                                    <div className="bg-secondary/50 dark:bg-secondary/50 border border-border dark:border-border rounded-xl p-4 grid gap-4">
                                      ...
                                      <div className="space-y-1">
                                        ...
                                        <Select
                                          value={editingBook.branchId || "all"}
                                          onValueChange={(value) =>
                                            setEditingBook({
                                              ...editingBook,
                                              branchId: value,
                          ...
                          <Button
                            ...
                            onClick={async () => {
                              try {
                                ...
                                setBooksData(
                                  booksData.filter((b) => b.id !== book.id),
                                );
                              } catch (err) {
                        ...
                        <Popover>
                          <PopoverTrigger asChild>
                            ...
                            >
                              {issueForm.studentId
                                ? students.find(
                                    (s) => String(s.id) === issueForm.studentId,
                                  )?.name
                                : "Select Student"}
                        ...
                        <Popover>
                          <PopoverTrigger asChild>
                            ...
                            >
                              {issueForm.bookId
                                ? books.find(
                                    (b) => String(b.id) === issueForm.bookId,
                                  )?.title
                                : "Select Book"}
                              <ChevronsUpDown className="ml-2 h-4 w-4 opacity-50" />
                            </Button>
