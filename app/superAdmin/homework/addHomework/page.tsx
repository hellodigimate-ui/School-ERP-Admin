...
import { Search, Eye, BookOpen, Calendar, Clock, Users } from "lucide-react";
...
const defaultStats = [
  {
    ...
    value: "0",
    icon: BookOpen,
    color: "bg-gradient-to-br from-blue-500/20 to-indigo-500/20 text-blue-700",
    iconBg:
  ...
  },
  {
    ...
    value: "0",
    icon: Calendar,
    color: "bg-gradient-to-br from-red-500/20 to-rose-500/20 text-red-700",
    iconBg: "bg-gradient-to-br from-red-500 to-rose-500 text-white shadow-md",
  },
  {
...
const Page = () => {
  ...
  const transformHomework = (item: any): HomeworkItem => ({
    ...
    total: item.total ?? 0,
    branchName: item.branch?.name ?? item.branchName ?? "-",
  ...
  const loadStats = async () => {
    try {
      ...
      const response = await axiosInstance.get(
        `/api/v1/homework/stats${query ? `?${query}` : ""}`,
      );
      if (!response || response.status !== 200) {
