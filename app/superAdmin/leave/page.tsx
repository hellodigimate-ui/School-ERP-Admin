...
export default function Page() {
  ...
  const fetchStats = async () => {
    try {
      ...
      const res = await axiosInstance.get("/api/v1/leave/requests/stats", {
        params,
      });
  ...
  useEffect(() => {
    fetchLeaves();
    fetchStats();
  }, [statusFilter, page, selectedBranchId, localBranch]);
      ...
      <div className="bg-white dark:bg-zinc-900 rounded-2xl border shadow-md p-6">
        <div className="flex flex-wrap items-center gap-4 justify-between mb-4">
          <div className="flex flex-wrap items-center gap-4">
            ...
            <div>
              <p className="text-sm font-medium mb-2">Branch</p>
              <Select
                value={localBranch}
          ...
          {isGlobalBranchSelected ? (
            <p className="text-sm text-muted-foreground">
              Global branch selection is active. Local branch filter is
              disabled.
            </p>
          ) : null}
        </div>
            ...
            {loading ? (
              ...
              leaveRequests.map((leave) => (
                <TableRow key={leave.id}>
                  ...
                  <TableCell>{leave.user?.role}</TableCell>
