...
const Page = () => {
      ...
      if (data.success) {
        setStats([
          ...
          },
          {
            ...
            value: data.data.outOfStockItems,
            icon: Warehouse,
            gradient: "from-emerald-500 to-teal-500",
          },
          {
  ...
  useEffect(() => {
    ...
    fetchItems();
  }, [searchTerm, category, branchFilter]);
  ...
  const handleAddItem = async () => {
    try {
      ...
      const selectedBranchIdForItem =
        newItem.branchId && newItem.branchId !== "all"
          ? newItem.branchId
          : branchFilter;
      if (selectedBranchIdForItem) {
        payload.branchId = selectedBranchIdForItem;
      }
  ...
  const handleDeleteIssue = async (id: string) => {
    try {
      const res = await axiosInstance.delete(`/api/v1/items/issues/${id}`);

      if (res.data.success) {
        setIssuedItems((prev) => prev.filter((issue) => issue.id !== id));
      ...
      <div className="space-y-6">
        <div>
          ...
          <p className="text-muted-foreground mt-1">
            Manage items, stores, issue & track stock levels
          </p>
        </div>
              ...
              <Dialog open={isAddOpen} onOpenChange={setIsAddOpen}>
                <DialogContent>
                  ...
                  <div className="space-y-2">
                    ...
                    <label>Branch</label>
                    <Select
                      value={newItem.branchId || branchFilter || "all"}
                      onValueChange={(value) =>
                        setNewItem({ ...newItem, branchId: value })
                      }
                      disabled={isGlobalBranchSelected}
                    >
                      <SelectTrigger>
                            ...
                            <DialogContent>
                              ...
                              {currentItem && (
                                <div className="space-y-4">
                                  ...
                                  <label>Branch</label>
                                  <Select
                                    value={currentItem.branchId || "all"}
                                    onValueChange={(value) =>
                                      setCurrentItem({
                                        ...currentItem,
                                        branchId: value,
                      ...
                      <Popover>
                        <PopoverTrigger asChild>
                          ...
                          >
                            {issueForm.userId
                              ? users.find(
                                  (u) => String(u.id) === issueForm.userId,
                                )?.name
                              : "Select Student"}
                        ...
                        <PopoverTrigger asChild>
                          ...
                          >
                            {issueForm.itemId
                              ? items.find(
                                  (item) =>
                                    String(item.id) === issueForm.itemId,
                                )?.name
                              : "Select Item"}
                ...
                <TableBody>
                  {issuedItems.map((issue) => (
                    <TableRow key={issue.id}>
                      ...
                      <TableCell>
                        {issue.returnDate ? issue.returnDate.slice(0, 10) : "-"}
                      </TableCell>
